...
Python 표준 라이브러리만을 활용하여 구현한 **파일 기반 콘솔 가계부 애플리케이션**입니다.

거래 내역과 카테고리, 월별 예산을 JSONL 파일에 영구 저장하며,
Generator 기반 스트리밍 처리와 Atomic File Replacement를 적용하여
대용량 데이터 처리와 파일 데이터의 안정성을 고려했습니다.

---

## 🎯 Project Goal

이 프로젝트는 단순한 콘솔 가계부 구현을 넘어
Python 표준 라이브러리를 활용하여 실제 애플리케이션에서 필요한
**데이터 저장, 계층 분리, 예외 처리, 데이터 무결성** 등을 직접 구현하는 것을 목표로 합니다.
...
거래 저장 시 TX-000001과 같은 고유 ID가 생성됩니다.
...
다른 저장 위치를 사용하려면 --data-dir 옵션을 사용할 수 있습니다.
...
이 방식을 통해 작업 중 프로그램이 종료되더라도
기존 파일이 중간 상태로 변경되는 문제를 줄일 수 있습니다.

🛡️ Exception Handling

애플리케이션 내부의 예외를 그대로 노출하지 않고
데코레이터를 통해 사용자 친화적인 오류 메시지를 제공합니다.
...
Import 과정에서 날짜, 타입, 카테고리, 금액 및 필수 컬럼을 검증합니다.
